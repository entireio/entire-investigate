...
// TestSaveInvestigateConfig_WritesOwnFile verifies that saveInvestigateConfig
// persists into the plugin's own .entire/investigate.local.json and leaves the
// CLI's settings files alone. The plugin used to write its config into
// .entire/settings.local.json under an "investigate" key, which coupled it to
// a file the CLI decodes with DisallowUnknownFields.
//
...
func TestSaveInvestigateConfig_WritesOwnFile(t *testing.T) {
	...
	own, err := os.ReadFile(filepath.Join(tmp, ".entire", config.FileName))
	require.NoError(t, err)
	require.Contains(t, string(own), `"agents"`)
	require.Contains(t, string(own), `"claude-code"`)

	// Neither CLI settings file may gain an investigate key.
	for _, name := range []string{"settings.json", "settings.local.json"} {
		data, err := os.ReadFile(filepath.Join(tmp, ".entire", name))
		if err != nil {
			continue
		}
		require.NotContains(t, string(data), `"investigate"`,
			"%s must not be touched by the plugin", name)
	}
}
