...
func TestNewCommand_AgentsFlagOverrideUsed(t *testing.T) {
	...
	if err := saveInvestigateSettings(&config.Config{
...
// TestNewCommand_ContinueLoadsAlwaysPromptFromConfig verifies that the
// configured always_prompt is reloaded on resume —
// without this, a Ctrl+C plus --continue silently loses the user's
// "be skeptical, cite line numbers"-style preamble.
func TestNewCommand_ContinueLoadsAlwaysPromptFromConfig(t *testing.T) {
	tmp := setupInvestigateRepo(t)
...
// TestNewCommand_ContinueWarnsOnConfigLoadFailure verifies that a corrupt
// config file on resume surfaces a visible warning instead of silently
// dropping the configured AlwaysPrompt. Without this, a user who breaks their
// config mid-run would see the agent's behaviour change with no explanation.
func TestNewCommand_ContinueWarnsOnConfigLoadFailure(t *testing.T) {
	tmp := setupInvestigateRepo(t)

	// Write a malformed config file so config.Load fails.
	if err := os.MkdirAll(filepath.Join(tmp, ".entire"), 0o750); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(tmp, ".entire", config.FileName), []byte("{broken-json"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(errBuf.String(), "could not reload config on --continue") {
		t.Errorf("stderr should warn about config load failure; got: %s", errBuf.String())
	}
	if captured.AlwaysPrompt != "" {
		t.Errorf("AlwaysPrompt = %q, want empty when config unavailable", captured.AlwaysPrompt)
	}
}
...
// saveInvestigateSettings persists cfg into the CWD's
// .entire/investigate.local.json, the file the production save path targets.
// It must stay untracked: always_prompt is dropped from a tracked file (see
// config.VerifyUntracked).
func saveInvestigateSettings(cfg *config.Config) error {
	return config.Save(context.Background(), cfg)
}
...
func TestRunFresh_SkipsMultipickerWhenAgentsFlagPresent(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(
		filepath.Join(tmp, ".entire", config.FileName),
		[]byte(`{"agents":["claude-code","codex"]}`), 0o644))
...
func TestRunInvestigate_SoftWarnAcceptedRunsLoop(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(
		filepath.Join(tmp, ".entire", config.FileName),
		[]byte(`{"agents":["claude-code"],"max_turns":1}`), 0o644))
